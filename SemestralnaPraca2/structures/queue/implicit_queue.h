...
namespace structures
{
	...
	inline ImplicitQueue<T>& ImplicitQueue<T>::operator=(const ImplicitQueue<T>& other)
	{
		if (this != &other)
		{
			...
			{
				if (other.startIndex_ + other.size_ <= array_->size()) // fixme 
				{
					Array<T>::copy(*other.array_, other.startIndex_, *array_, 0, static_cast<int>(other.size_));
				}
				else
				{
					Array<T>::copy(*other.array_, other.startIndex_, *array_, 0, static_cast<int>(other.array_->size()) - other.startIndex_);
					Array<T>::copy(*other.array_, 0, *array_, static_cast<int>(other.array_->size()) - other.startIndex_, static_cast<int>(other.size_) - (static_cast<int>(other.array_->size()) - other.startIndex_));
				}
				startIndex_ = 0;
				size_ = other.size_;
			}
		...
		throw std::out_of_range("ImplicitQueue<T>:: Queue sa rovnaju!");
	}
