...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		...
		{
			throw std::logic_error("TreeNode<T>::getRoot: Node is a root!");
		}
	}
	...
	{
		std::queue<TreeNode<T>*>* helpQueue = new std::queue<TreeNode<T>*>();

		helpQueue->push(current);

		while (!helpQueue->empty())
		{
			TreeNode<T>* node = helpQueue->pop();

			if (node != nullptr)
			{
				path_->push(node); 

				for (int i = 0; i < node->degree(); i++) // NOTE current->degree()
				{
					helpQueue->push(node->getSon(i)); // NOTE current->degree()
				}
			}
		}
		delete helpQueue;
