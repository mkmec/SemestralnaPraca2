...
namespace structures {
	...
	void Vector::copy(const Vector& src, const int srcStartIndex, Vector& dest, const int destStartIndex, const int length)
	{
		if (length > 0) {
			...
			DSRoutines::rangeCheckExcept(srcStartIndex, src.size_, "Invalid index in src vector!");
			DSRoutines::rangeCheckExcept(destStartIndex, dest.size_, "Invalid index in dest vector!");
			DSRoutines::rangeCheckExcept(length + srcStartIndex, src.size_ + 1, "Invalid end index in src vector!");
			DSRoutines::rangeCheckExcept(length + destStartIndex, dest.size_ + 1, "Invalid end index in dest vector!");
